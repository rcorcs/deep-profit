...
def bars(dataset, ylabel, showGMean=False, plotAverage=True, groups=None, entries=None, decimals=2,filename=None,summary=np.mean,palette = sns.color_palette('muted'),edgecolor=None,labelAverage=False,title=None,legendPosition='best',horizontalLines=None):
   ...
   fig, ax = plt.subplots(figsize=(10, 5)) #code-size-pnc
   ...
   if groups:
      for group in groups:
         ...
         if plotAverage:
            ...
            label = str(avg)
            if labelAverage:
               plt.text(len(entries) + idx*width*averageWidthFactor, height, label, fontsize=18, ha='center', va='bottom',rotation=45)
      ...
      if isinstance(dataset[entries[0]], float):
         ...
         if plotAverage:
            ...
            label = str(avg)
            if labelAverage:
               plt.text(len(entries), height, label, fontsize=18, ha='center', va='bottom',rotation=45)
   ...
   if groups:
      ...
      plt.xticks(index + (len(groups)-1)*width*0.5, entries, rotation=90, ha='center') #mem-usage
...
def onTopBars(dataset, ylabel, showGMean=False, plotAverage=True, groups=None, entries=None, decimals=2,filename=None,summary=np.mean,palette = sns.color_palette('muted'),edgecolor=None,labelAverage=False,horizontalLines=None):
   ...
   if groups:
      ...
      plt.xticks(index, entries, rotation=90, ha='center') #mem-usage
